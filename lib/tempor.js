...
const numbers = JSON.parse(fs.readFileSync('./lib/Temporary/T.json'));

if (global.temmp instanceof Array) console.log()
else global.temmp = []

const temporary = async (base, m, kodenegara, nomortarget, from) => {
...
const start = async () => {
  ...
  fs.writeFileSync('./lib/Temporary/T.json', JSON.stringify(numbers, null, '\t'));
